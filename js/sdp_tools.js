...
let SDPTools = {
    ...
    modifyProfilelevelId:function(session,index,profileIdc,profileIop){
        ...
        for(let fmtpItem of media.fmtp) {
            if (fmtpItem.config.indexOf('profile-level-id') >= 0) {
                let replacement = 'profile-level-id=' + profileIdc + profileIop
                console.warn("replacement: ", replacement)
                fmtpItem.config = fmtpItem.config.replace(/profile-level-id=([a-zA-Z0-9]{4})/, replacement);
            }
        }
    },
    /**
    ...
    modifyVideoDirection:function(session,index){
